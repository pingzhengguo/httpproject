...
public class HttpCoon {
     ...
     * @return
     */
    public static String httpURLConnectionJSONAndHeader(String urlStr, Map<String, Object> paramMap, Map<String, String> headerMap) throws CustomException {
        HttpURLConnection connection = null;
        ...
        try {
            ...
            if (paramMap != null) {
                // 创建输入输出流,用于往连接里面输出携带的参数,(输出内容为?后面的内容)
                dataout = new PrintWriter(new OutputStreamWriter(connection.getOutputStream(), "utf-8"));
                System.out.println("请求参数：" + JSON.toJSONString(paramMap));
                dataout.println(JSON.toJSONString(paramMap));
                // 输出完成后刷新并关闭流
                dataout.flush();
            }
            // 连接发起请求,处理服务器响应  (从连接获取到输入流并包装为bufferedReader)
            try {
                ...
                bf = new BufferedReader(new InputStreamReader(connection.getErrorStream(), "UTF-8"));
                String line;
                // 用来存储响应数据
                StringBuilder sb = new StringBuilder();
                // 循环读取流,若不到结尾处
